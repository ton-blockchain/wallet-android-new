...
#define FUNC(FUNC_NAME) Java_org_ton_wallet_rlottie_RLottieDrawable_##FUNC_NAME
...
FUNC(create)(JNIEnv *env, jclass clazz, jstring src, jintArray data, jboolean precache, jintArray colorReplacement) {
    ...
    if (colorReplacement != nullptr) {
        ...
        if (arr != nullptr) {
            ...
            env->ReleaseIntArrayElements(colorReplacement, arr, JNI_OK);
        }
    }
...
extern "C"
JNIEXPORT jlong JNICALL
FUNC(createWithJson)(JNIEnv *env, jclass clazz, jstring json, jstring name, jintArray data) {
...
FUNC(destroy)(JNIEnv *env, jclass clazz, jlong ptr) {
    if (ptr == NULL) {
        return;
    }
...
extern "C"
JNIEXPORT void JNICALL
FUNC(setLayerColor)(JNIEnv *env, jclass clazz, jlong ptr, jstring layer, jint color) {
...
extern "C"
JNIEXPORT void JNICALL
FUNC(createCache)(JNIEnv *env, jclass clazz, jlong ptr, jobject bitmap, jint w, jint h, jint stride) {
    ...
    if (info->nextFrameIsCacheFrame && info->createCache && info->frameCount != 0 && w * 4 == stride && AndroidBitmap_lockPixels(env, bitmap, &pixels) >= 0) {
        ...
        if (precacheFile != nullptr) {
            ...
            for (size_t a = 0; a < info->frameCount; a += framesPerUpdate) {
                ...
                info->maxFrameSize = std::max(info->maxFrameSize, size);
...
FUNC(getFrame)(JNIEnv *env, jclass clazz, jlong ptr, jint frame, jobject bitmap, jint w, jint h, jint stride) {
    if (ptr == NULL || bitmap == nullptr) {
        return 0;
    }
    LottieInfo *info = (LottieInfo *) (intptr_t) ptr;
