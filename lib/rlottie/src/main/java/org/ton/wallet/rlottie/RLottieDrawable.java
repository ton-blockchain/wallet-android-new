...
public class RLottieDrawable extends BitmapDrawable implements Animatable {
    ...
    public RLottieDrawable(Context context, int rawRes, String name, int w, int h) {
        this(context, rawRes, name, w, h, true);
    }

    public RLottieDrawable(Context context, int rawRes, String name, int w, int h, boolean startDecode) {
        try {
    ...
    public RLottieDrawable(String json, String name, int width, int height, boolean startDecode) {
        this.nativePtr = createWithJson(json, name, metaData);
        this.width = width;
        this.height = height;
        this.timeBetweenFrames = Math.max(16, (int) (1000.0f / metaData[1]));
        this.autoRepeat = 0;
        if (startDecode) {
            setAllowDecodeSingleFrame(true);
        }
