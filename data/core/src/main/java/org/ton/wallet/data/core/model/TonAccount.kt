...
class TonAccount(
    ...
    fun getCode(): ByteArray {
        val codeString = when (TonAccountType.getAccountType(version, revision)) {
    ...
    fun getInitialData(): ByteArray {
        val dataCell = getDataCell(0)
        return BagOfCells(dataCell).toByteArray()
    }
    ...
    private fun getDataCell(seqNo: Int): Cell {
